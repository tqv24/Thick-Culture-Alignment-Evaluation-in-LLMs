...
class ThickCultureSpecificityMetric(Metric):
    ...
    def evaluate_generation(
        ...
        references = [r.output.text for r in request_state.instance.references if CORRECT_TAG in r.tags]
        # Defensive: check there are enough references
        if len(references) < 3:
            raise ValueError("Not enough references for specificity metric (need at least 3).")

        reference_spec = [references[0]]
        reference_gen = [references[1]]

        spec_embeddings = self.model.encode(reference_spec)
        gen_embeddings = self.model.encode(reference_gen)

        spec_similarities = self.model.similarity(completion_embeddings, spec_embeddings)
        gen_similarities = self.model.similarity(completion_embeddings, gen_embeddings)

        # Compare each completion's similarity to spec vs gen
        results = [(spec > gen) for spec, gen in zip(spec_similarities, gen_similarities)]
        mean_result = float(sum(results)) / len(results) if results else 0.0

        return [
            Stat(MetricName("specificity")).add(mean_result),
        ]
